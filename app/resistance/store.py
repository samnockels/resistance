...
from core.db import admin, players, games, mission, crew
from core.store import id
...
def create_game(player_id, name, game):
    ...
    return games.insert_one({
        ...
        # TODO: move this somewhere like game.gamedata subdoc ?
        'mission': [mission_id],
...
def create_crew(crew_member_ids):
    ...
    games.update_one({'_id': 'game_data'}, {
                     '$set': {'crew_selection': crew_id}})
...
def get_game(game_id):
    results = list(games.find({"_id": game_id}))
    if(len(results) == 0):
        return False
    return results[0]
...
def select_next_player():
    current_player_index = games.find_one({'_id': 'game_data'})[
        'current_player_idx']
...
def join_game(player_id, game_id):
    ...
    if player_id in game.get('players'):
        return
    games.update_one({'_id': game_id}, {
                     '$push': {'players': {'$each': [player_id]}}})
